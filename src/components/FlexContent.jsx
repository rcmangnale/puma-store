      ...
      <div className={`flex items-center justify-between lg:flex-col lg:justify-center puma-container ${ifExists ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
          <img
            src={img}
            alt={`img/${heading}`}
            className={`w-auto object-fill transitions-theme ${ifExists ? 'h-80 lg:h-80 md:h-52 sm:h-44 xsm:h-36 rotate-6 hover:-rotate-12' : 'h-72 lg:h-64 md:h-60 sm:h-48 xsm:h-40  hover:rotate-12'}`}
          />
        </div>
      </div>
   </>
